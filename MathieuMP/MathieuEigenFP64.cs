...
namespace MathieuMP {
    public static class MathieuEigenFP64 {
        ...
        public static (double value, double error) SearchFitA(int n, double q, double a, int maxiter = 16) {
            if (q == 0) {
                return (0d, 0d);
            }

            double a0 = a, d0 = FractionA(n, q, a0), da = double.NaN;

            for (int iter = 0; iter < maxiter; iter++) {
                double ah = Math.CopySign(Math.Max(Math.ScaleB(1, -40), Math.ScaleB(Math.Abs(a0), -10)), a0);
                double a1 = a0 + ah, d1 = FractionA(n, q, a1);
        ...
        // The true value is obtained by the secant method.
        public static (double value, double error) SearchFitB(int n, double q, double a, int maxiter = 16) {
            if (q == 0) {
                return (0d, 0d);
            }

            double a0 = a, d0 = FractionB(n, q, a0), da = double.NaN;

            for (int iter = 0; iter < maxiter; iter++) {
                double ah = Math.CopySign(Math.Max(Math.ScaleB(1, -40), Math.ScaleB(Math.Abs(a0), -10)), a0);
                double a1 = a0 + ah, d1 = FractionB(n, q, a1);
