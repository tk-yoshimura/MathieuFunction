...
namespace MathieuMP {
    class Program {
        static void Main() {
            ...
            for (MultiPrecision<Pow2.N4> u = MultiPrecision<Pow2.N4>.Ldexp(1, -128); u <= 256; u *= 2) {
                us.Add(u);
            }
            ...
            for (int n = 2; n <= 64; n++) {
                ...
                using StreamWriter sw = new($"../../../../results/eigen_nearzero_log2_precision1024bits_n{n}.csv");
                ...
                using BinaryWriter bw = new(File.OpenWrite($"../../../../results/eigen_nearzero_log2_precision1024bits_n{n}.bin"));
                ...
                foreach (MultiPrecision<Pow2.N4> u in us) {
                    ...
                    (MultiPrecision<Pow2.N32> m, MultiPrecision<Pow2.N32> d, bool cancellation_digits, mp_length)
                        = ComputeDigits1024bits(n, q, mp_length);
        ...
        static (MultiPrecision<Pow2.N32> m, MultiPrecision<Pow2.N32> d, bool cancellation_digits, int mp_length) ComputeDigits1024bits(int n, MultiPrecision<Pow2.N32> q, int mp_length) {
            if (n == 0) {
                return (EigenMP<Pow2.N32>.Value(EigenFunc.A, n, q, zero_shift: true).value, 0, cancellation_digits: false, mp_length: 0);
            }
            ...
            if (mp_length <= 36) {
                ...
                if (!cancellation_digits) {
                    return (m, d, cancellation_digits, 36);
                }
            }
            ...
            if (mp_length <= 40) {
                ...
                if (!cancellation_digits) {
                    return (m, d, cancellation_digits, 40);
                }
            }
            ...
            if (mp_length <= 48) {
                ...
                if (!cancellation_digits) {
                    return (m, d, cancellation_digits, 48);
                }
            }
            ...
            if (mp_length <= 64) {
                ...
                if (!cancellation_digits) {
                    return (m, d, cancellation_digits, 64);
                }
            }
